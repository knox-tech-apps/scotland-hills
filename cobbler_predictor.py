...
"""
The Cobbler Summit Weather Predictor
====================================

Usage:
    python cobbler_predictor.py
...
COBBLER = {
     "name": "The Cobbler (Ben Arthur) Summit",
    "lat":  56.3647,
    "lon":  -4.7618,
    "elevation": 884,
             # metres
...
def fetch_historical(client, years: int = HISTORY_YEARS) -> pd.DataFrame:
    ...
    params = {
        "latitude":  COBBLER["lat"],
        "longitude": COBBLER["lon"],
        "elevation": COBBLER["elevation"],
...
def plot_cv_scores(cv_scores):
    ...
    fig.suptitle("Cross-Validation Performance — Cobbler Summit Models",
...
def plot_feature_importance(models, features, top_n=15):
    ...
    fig.suptitle("Top Feature Importances — Cobbler Summit Models",
...
def plot_forecast(forecast_df: pd.DataFrame, predictions: pd.DataFrame):
    ...
    fig.suptitle(
        f"14-Day Forecast — {COBBLER['name']} ({COBBLER['elevation']}m)\n"
...
def plot_seasonal_climatology(df: pd.DataFrame):
    ...
    fig.suptitle(f"Monthly Climatology — {COBBLER['name']} (last {HISTORY_YEARS} years)",
                 fontsize=14, fontweight="bold")
...
def safety_summary(predictions: pd.DataFrame):
    ...
    print("\n" + "═"*55)
    print("  🏔️   COBBLER SUMMIT — 14-DAY SAFETY SUMMARY")
    print("═"*55)
...
def main():
    print("="*55)
    print("  COBBLER SUMMIT WEATHER PREDICTOR")
    print(f"  Lat: {COBBLER['lat']}°N  Lon: {COBBLER['lon']}°W  "
          f"Elev: {COBBLER['elevation']}m")
    print("="*55 + "\n")
