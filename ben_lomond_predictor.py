"""
Ben Lomond Summit Weather Predictor
====================================
Fetches historical weather data from Open-Meteo for the Ben Lomond summit
(56.1915°N, 4.6346°W, 974m), engineers features, trains Random Forest models,
evaluates them, and generates a 7-day forecast.
...
BEN_LOMOND = {
    "name": "Ben Lomond Summit",
    "lat": 56.1915,
    "lon": -4.6346,
    "elevation": 974,          # metres
}
...
def fetch_historical(client, years: int = HISTORY_YEARS) -> pd.DataFrame:
    ...
    params = {
        "latitude":  BEN_LOMOND["lat"],
        "longitude": BEN_LOMOND["lon"],
        "elevation": BEN_LOMOND["elevation"],
...
def plot_cv_scores(cv_scores):
    ...
    fig.suptitle("Cross-Validation Performance — Ben Lomond Summit Models",
...
def plot_feature_importance(models, features, top_n=15):
    ...
    fig.suptitle("Top Feature Importances — Ben Lomond Summit Models",
...
def plot_forecast(forecast_df: pd.DataFrame, predictions: pd.DataFrame):
    ...
    fig.suptitle(
        f"14-Day Forecast — {BEN_LOMOND['name']} ({BEN_LOMOND['elevation']}m)\n"
...
def plot_seasonal_climatology(df: pd.DataFrame):
    ...
    fig.suptitle(f"Monthly Climatology — {BEN_LOMOND['name']} (last {HISTORY_YEARS} years)",
                 fontsize=14, fontweight="bold")
...
def safety_summary(predictions: pd.DataFrame):
    ...
    print("\n" + "═"*55)
    print("  🏔️   BEN LOMOND SUMMIT — 14-DAY SAFETY SUMMARY")
    print("═"*55)
...
def main():
    print("="*55)
    print("  BEN LOMOND SUMMIT WEATHER PREDICTOR")
    print(f"  Lat: {BEN_LOMOND['lat']}°N  Lon: {BEN_LOMOND['lon']}°W  "
          f"Elev: {BEN_LOMOND['elevation']}m")
    print("="*55 + "\n")
